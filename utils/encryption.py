...
class PasswordHandler:
    ...
    def __set_params(self):
        secret_key = aws_keys.ParamsHandler().get_secure_param(
            self.settings.password_secure_path
        )
    ...
    def encrypt(self):
        fernet = Fernet(
            base64.urlsafe_b64encode(binascii.unhexlify(self.__encryption_key))
        )
...
class JWTHandler:
    ...
    def generate(self):
        self.payload["exp"] = datetime.datetime.now(
            tz=datetime.timezone.utc
        ) + datetime.timedelta(minutes=self.EXPIRE_TIME_IN_MINUTES)
